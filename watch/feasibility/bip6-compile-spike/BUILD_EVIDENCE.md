...
On 2026-09-03, the Project Owner installed version 0.0.1 through Zepp
Developer Mode on a physical Amazfit Bip 6. Supplied evidence showed the app in
the watch application list and showed it running with the expected
`Amazfit Bip 6`, `Compile feasibility only`, and `No medical function` text.

**Result: PASS** for installation and foreground launch only. The photographs
and detailed evidence record are retained privately; they are not published in
this source repository. Watch firmware and Zepp mobile-app versions remain to
be recorded.
...
- repeated launch, reboot persistence, and uninstall/reinstall behavior on a
